...
sudo docker run --gpus all -p 3000:3000 -v huggingface_models:/root/.cache/huggingface  -e PYANNOTE_AUTH_TOKEN='VAD_TOKEN_HERE' voicestreamai
```
...
```bash
sudo docker run --gpus all -p 3000:3000 -e PYANNOTE_AUTH_TOKEN='VAD_TOKEN_HERE' voicestreamai
```

#### New
In local machine run
```bash
tar --exclude='.*' -czf code.tar.gz asr
scp -r code.tar.gz 10.90.126.61:/home/CORP/kunal259787
```
...
cd asr
docker build -t asr_test .
docker run --gpus all asr_test
```
...
#### Milestones
  - Choose best model for offline processing based on inference on benchmark data, and some real case data like Exelia
    - Also manuall check if transcription makes sense by seeing if numbers and all are converted properly
  - Benchmarking UI to share with leadership -> Formatting the table/UI, Comments
  - Integrate with a project and prepare demo -> what project?
  - TTS -> 11 lapse
  - Real-time use case: Focus on improving latency along with other aspects of real-time ASR
  - Technical Debt:
    - Automate benchmark running code
    - Remove unused/dead/commented-out code from ASR
    - use config files


## Runbook
- If 'ssh <machine-ip>' fails:
  - check if it's failing only for you, and if yes, then try after restarting the system.  If it still fails, then email Digital.Services@exlservice.com for help
  - update netskope and retry
  - restart the machine from AWS web-console
    - after restart:
      - ssh 10.90.126.61
      - cd /home/CORP
      - ./start_scripts.sh
  - As a workaround you can ssh to a different machine, and indirectly ssh to the target machine from there
...
## projects
- asr_realtime -> exposes websocket, real-time streaming
  - riva
  - whisper
- asr_offline -> offline transcription
  - riva
- speech-to-speech
...
```
- run as sudo -> sudo su
- mkdir -p docker_context/models--pyannote--segmentation/snapshots
- docker build --no-cache -t asr_realtime .
- docker run --gpus all -d -p 3000:3000 asr_realtime

```

## Config

If CONFIG_FILE is set → loads that directly

Else if ENV is set → picks dev.yaml, prod.yaml

Else → fallback to dev.yaml
...
docker run --rm -d -p 3000:3000 -v $(pwd)/logs:/app/logs -e ENV=dev_aicoe_10_90_126_61 asr_realtime

## Improvement
- remove "send_riva_mutation_to_db" and other save_to_db options and directly only provide the transcripts in realtime to Lambda function "connectRiva" only.  Save to db should be done in "connectRiva" lambda function.
  - The values saved to db is used by another lambda "lex_llm_config" which polls the DB every 1.25 seconds
  - These are the 3 lambdas using asr_realtime: startKVS, ConnectTIVA, LEX_LLM_CONFIG

## Amelia Integration with Riva
- POC: Delivery team

## To create reconizer for google stt
...
## To download google_credentials
from google.cloud import storage
client = storage.Client() 
bucket = client.bucket("crun-srvc-acc-cred")
blob = bucket.blob("eci-ugi-digital-ccaipoc-e95ef5cd3c3e.json")
blob.download_to_filename("google_credential.json")
print("Downloaded to credential.json")

## Custom Ingress in 10.90.126.61 machine
- ./TTS/riva_tts_test_api_v2.py
  - In git we maintain it under util/miscellaneous/riva_tts_test_api_v2.py
  - to restart this fastapi server after changes run this code: 
    - tmux attach-session -t tts
    - tmux new-session -t tts (if closed)
    - ./home/CORP/start_riva_api.sh
    - ctrl+b then press d

## docker images
- nvcr.io/nvidia/riva/riva-speech:2.19.0 and redis docker images are out-of-the-box ones.
- The docker images we build are: asr_offline:latest, asr_realtime:latest and riva_finetuned_inspira:latest

## Test
- run test/asr_realtime/inference.py